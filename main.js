...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "ProyectoXBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
let prekey = []
let directorio = readdirSync("./ProyectoX")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./ProyectoX/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./ProyectoXBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./ProyectoXBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./ProyectoXBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./ProyectoXBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./ProyectoX/', './ProyectoXBot/']
